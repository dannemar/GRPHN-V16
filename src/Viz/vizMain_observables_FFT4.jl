...
#
# !!! NB !!!
# Julia fftfreq calcs f in Hz. To convert to \omega: \omega = 2\pi f
#
# =#
...
using Printf
using FFTW
using Mmap # Added for memory-mapped I/O

function process_observable(file_path::String, R::Int)
    # Determine the number of Float64 elements based on the file size
    file_bytes = filesize(file_path)
    total_elements = div(file_bytes, sizeof(Float64))
    T_steps = div(total_elements, R)

    # Open the file and memory-map it directly into a matrix
    io = open(file_path, "r")
    mat = Mmap.mmap(io, Matrix{Float64}, (R, T_steps))

    # Compute statistics directly from the memory-mapped array
    mean_vals = vec(mean(mat, dims=1))
    std_vals = vec(std(mat, dims=1))
    sem_vals = std_vals ./ sqrt(R)

    # Close the IO stream; the memory map will be handled by the garbage collector
    close(io)
...
function plot_all_observables()
    ...
    # Figure Setup: 4 Rows x 2 Columns (Width adjusted from 3600 to 2400)
    ...
    # Define axes for frequency-domain with peaks (Column 2)
    ...
    for (idx, (w, h, energy)) in enumerate(lattice_configs)
        ...
        # Plot Column 2: Frequency Domain Overlayed with Peaks and Annotations
    ...
    # Save and display (updated filename to 4x2)
    save("../../img/AB_viz_obsrv_LINLIN_FFTW_dt-$(dt_str).png", fig)
    display(fig)
end
