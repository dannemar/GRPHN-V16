...
function extract_all_peaks(magnitudes, frequencies)
    ...
    for k in 2:(length(magnitudes)-1)
        # 1. Strict local maximum check only
...
function plot_all_observables()
    ...
    ax_ce_fft = Axis(fig[1, 2], xlabel=L"\omega", ylabel="Magnitude FT(C_E)", xlabelsize = 42)
    ax_cd_fft = Axis(fig[2, 2], xlabel=L"\omega", ylabel="Magnitude FT(C_D)", xlabelsize = 42)
    ax_cv_fft = Axis(fig[3, 2], xlabel=L"\omega", ylabel="Magnitude FT(C_v)", xlabelsize = 42)
    ax_crel_fft = Axis(fig[4, 2], xlabel=L"\omega", ylabel="Magnitude FT(C_rel)", xlabelsize = 42)
    ...
    for (idx, (w, h, energy)) in enumerate(lattice_configs)
        ...
        println(repeat("-", 50))
        println("Extracted Peaks for Configuration: ", lbl)

        obs_data = [
            ("C_E", ce_fft_abs_pos),
            ("C_D", cd_fft_abs_pos),
            ("C_v", cv_fft_abs_pos),
            ("C_rel", crel_fft_abs_pos)
        ]

        for (name, mag_array) in obs_data
            # Truncate to the plotted visual range for relevant extraction
            view_mag = mag_array[1:freq_stop_cnt]
            view_freq = freqs_pos[1:freq_stop_cnt]

            peaks = extract_all_peaks(view_mag, view_freq)

            println("\nObservable: ", name)
            if isempty(peaks)
                println("  No distinct peaks found.")
            else
                for (p_idx, (f, m)) in enumerate(peaks)
                    @printf("  Peak %d: Frequency = %.4f Hz, Magnitude = %.2f\n", p_idx, f, m)
                end
            end
        end
        println(repeat("-", 50))
    ...
    # Save and display
    save("../../img/AB_viz_obsrv_LINLIN_FFTW-NoPeaks_4x2.png", fig)
end
