...
function main()
    ...
    # Process data
    # (Assuming file exists for execution, otherwise replace with synthetic data for testing)
    ...
    M = 64 # Number of expected dominant physical modes
    mpm_params = matrix_pencil_method(eq_crel, dt, M)
    ...
    ax1 = Axis(fig[1, 1],
              ...
              title="Matrix Pencil Method (MPM) Fit vs Original Data")
    ...
    # Link the x-axes so panning/zooming applies to both plots
    linkxaxes!(ax1, ax2)
