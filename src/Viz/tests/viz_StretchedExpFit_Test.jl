using GLMakie
using Statistics

# ==========================================
# Data Processing Function
# ==========================================
function process_observable(file_path::String, R::Int)
    raw_data = collect(reinterpret(Float64, read(file_path))) #[cite: 5]
    T_steps = div(length(raw_data), R) #[cite: 5]
    mat = reshape(raw_data, (R, T_steps)) #[cite: 5]

    mean_vals = vec(mean(mat, dims=1)) #[cite: 5]
    std_vals = vec(std(mat, dims=1)) #[cite: 5]
    sem_vals = std_vals ./ sqrt(R) #[cite: 5]

    return mean_vals, std_vals, sem_vals, T_steps #[cite: 5]
end

# ==========================================
# Two-Term Stretched Exponential Model
# ==========================================
function two_term_stretched(t, C0, A1, τ1, β1, ω1, ϕ1, A2, τ2, β2, ω2, ϕ2)
    term1 = A1 .* exp.(-((t ./ τ1) .^ β1)) .* cos.(ω1 .* t .+ ϕ1)
    term2 = A2 .* exp.(-((t ./ τ2) .^ β2)) .* cos.(ω2 .* t .+ ϕ2)
    return C0 .+ term1 .+ term2
end

# ==========================================
# Main Plotting Routine
# ==========================================
function interactive_two_term_fit()
    # 1. Configuration and Data Loading
    data_dir = "../../../data/" #[cite: 5]
    w, h, energy = 256, 256, 500 #[cite: 5]
    s, tt, eqt_str, dt_str, R, of = 41, 600, "500.000", "0.010", 200, 1 #[cite: 5]

    dt = parse(Float64, dt_str) #[cite: 5]
    eqt = parse(Float64, eqt_str) #[cite: 5]
    eq_idx = Int(ceil(eqt / dt)) + 1 #[cite: 5]

    strEnd = "_w-$(w)_h-$(h)_H-$(energy).00_s-$(s)_tt-$(tt)_eqt-$(eqt_str)_dt-$(dt_str)_r-$(R)_of-$(of)" #[cite: 5]
    file_path = joinpath(data_dir, "acf_C_rel" * strEnd * ".bin") #[cite: 5]

    # Process data
    mean_crel, _, _, T_steps = process_observable(file_path, R) #[cite: 5]
    t_phy = (1:T_steps) .* dt #[cite: 5]

    # Isolate equilibrated data
    t_eq = t_phy[eq_idx:end] #[cite: 5]
    t_fit = t_eq .- t_eq[1] #[cite: 5]
    eq_crel = mean_crel[eq_idx:end] #[cite: 5]
    mean_offset = mean(eq_crel) #[cite: 5]

    # 2. Figure Setup
    fig = Figure(size=(1400, 1000), fontsize=20) #[cite: 5]
    ax = Axis(fig[1, 1], xlabel="Time (shifted to t=0)", ylabel="Bond Length C_B(t)", title="Interactive Two-Term Stretched Exponential Fit")

    # 3. Interactive UI Elements (SliderGrid)
    sg = SliderGrid(fig[2, 1],
            (label = "Offset (C₀)", range = -0.5:0.001:0.5, startvalue = mean_offset),
            (label = "Amp₁ (A₁)", range = 0.0:0.001:1.0, startvalue = 0.074),
            (label = "Tau₁ (τ₁)", range = 0.1:0.1:200.0, startvalue = 10.0),
            (label = "Beta₁ (β₁)", range = 0.1:0.01:2.0, startvalue = 1.0),
            (label = "Freq₁ (ω₁)", range = 0:0.01:5.0, startvalue = 3.39),
            (label = "Phase₁ (ϕ₁)", range = -pi:0.01:pi, startvalue = 0.0),
            (label = "Amp₂ (A₂)", range = 0.0:0.001:0.2, startvalue = 0.043),
            (label = "Tau₂ (τ₂)", range = 0.1:0.1:200.0, startvalue = 10.0),
            (label = "Beta₂ (β₂)", range = 0.1:0.01:2.0, startvalue = 1.0),
            (label = "Freq₂ (ω₂)", range = 0:0.01:5.0, startvalue = 4.02),
            (label = "Phase₂ (ϕ₂)", range = -pi:0.01:pi, startvalue = 0.0)
        )

    # Extract observables from sliders
    C0_obs = sg.sliders[1].value
    A1_obs = sg.sliders[2].value
    τ1_obs = sg.sliders[3].value
    β1_obs = sg.sliders[4].value
    ω1_obs = sg.sliders[5].value
    ϕ1_obs = sg.sliders[6].value

    A2_obs = sg.sliders[7].value
    τ2_obs = sg.sliders[8].value
    β2_obs = sg.sliders[9].value
    ω2_obs = sg.sliders[10].value
    ϕ2_obs = sg.sliders[11].value

    # 4. Bind Observables to the Model
    fit_line = lift(C0_obs, A1_obs, τ1_obs, β1_obs, ω1_obs, ϕ1_obs, A2_obs, τ2_obs, β2_obs, ω2_obs, ϕ2_obs) do C0, A1, τ1, β1, ω1, ϕ1, A2, τ2, β2, ω2, ϕ2
        two_term_stretched(t_fit, C0, A1, τ1, β1, ω1, ϕ1, A2, τ2, β2, ω2, ϕ2)
    end

    # 5. Plotting
    lines!(ax, t_fit, eq_crel, color=:blue, linewidth=2.5, label="Original C_rel") #[cite: 5]
    lines!(ax, t_fit, fit_line, color=:red, linewidth=2.5, label="Two-Term Stretched Model")

    axislegend(ax, position=:rt) #[cite: 5]

    # 6. Display the interactive figure
    display(fig) #[cite: 5]
end

# Run the routine
interactive_two_term_fit()
