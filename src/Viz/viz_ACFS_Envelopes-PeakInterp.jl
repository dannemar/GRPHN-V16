...
using Statistics
using Printf
using Peaks          # Replaces DSP for identifying local maxima
using Interpolations # Added for constructing the envelope curve
...
function extract_upper_envelope(C_raw::Vector{Float64})
    ...
    # Convert to standard vectors to allow boundary modifications
    idx_vec = collect(Int, peak_indices)
    val_vec = collect(Float64, peak_values)

    # 2. Ensure boundaries are included to prevent extrapolation errors
    ...
    # 3. Interpolate a curve strictly through the positive peaks
    itp = linear_interpolation(idx_vec, val_vec)

    # 4. Evaluate over the entire time series length
    upper_envelope = itp(1:length(C_raw))
...
function plot_all_observables()
    ...
    for (idx, (w, h, energy)) in enumerate(lattice_configs)
        ...
        # Calculate empirical upper envelopes
        env_cd = extract_upper_envelope(data_cd)
        env_cv = extract_upper_envelope(data_cv)
        env_crel = extract_upper_envelope(data_crel)
        ...
        lines!(ax_auto_cd, t_phy[eq_idx:end], env_cd, color=:black, linestyle=:dash, linewidth=lwidth-1, label="Env C_D")

        # Plot Row 3: C_V
        lines!(ax_auto_cv, t_phy[eq_idx:end], data_cv, color=c, linewidth=lwidth, label="C_V ($lbl)")
        lines!(ax_auto_cv, t_phy[eq_idx:end], env_cv, color=colors[2], linestyle=:dash, linewidth=lwidth, label="Env C_V")

        # Plot Row 4: C_rel
        lines!(ax_auto_crel, t_phy[eq_idx:end], data_crel, color=c, linewidth=lwidth, label="C_rel ($lbl)")
        lines!(ax_auto_crel, t_phy[eq_idx:end], env_crel, color=colors[3], linestyle=:dash, linewidth=lwidth, label="Env C_rel")

        # Plot Row 5: Comparison Plot
        lines!(ax_comp, t_phy[eq_idx:end], data_ce, color=colors[1], linewidth=lwidth, label="C_E (Raw)")
        lines!(ax_comp, t_phy[eq_idx:end], env_cd, color=colors[2], linestyle=:dash, linewidth=lwidth, label="Env C_D")
        lines!(ax_comp, t_phy[eq_idx:end], env_cv, color=colors[3], linestyle=:dash, linewidth=lwidth, label="Env C_V")
        lines!(ax_comp, t_phy[eq_idx:end], env_crel, color=colors[4], linestyle=:dash, linewidth=lwidth, label="Env C_rel")
    end
    ...
    save("../../img/ACFs_LINLIN_Env-PeakInterp.png", fig)
end
